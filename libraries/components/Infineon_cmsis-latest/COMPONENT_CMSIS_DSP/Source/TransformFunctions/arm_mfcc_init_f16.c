...
arm_status arm_mfcc_init_##LEN##_f16(         \
  arm_mfcc_instance_f16 * S,                  \
  uint32_t nbMelFilters,                      \
  uint32_t nbDctOutputs,                      \
  const float16_t *dctCoefs,                  \
...
{                                             \
 ...
 S->windowCoefs=windowCoefs;                  \
                                              \
 status=arm_cfft_init_##LEN##_f16(&(S->cfft));\
                                              \
 return(status);                              \
}
#else
...
{                                                \
 ...
 S->windowCoefs=windowCoefs;                     \
                                                 \
 status=arm_rfft_fast_init_##LEN##_f16(&(S->rfft));\
                                                 \
 return(status);                                 \
}
#endif
